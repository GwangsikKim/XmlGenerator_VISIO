...
namespace XMLGeneratorVISIO
{
    class VisioReader
    {
        ...
        private readonly string[] PipeSymbolTypes =
        {
            ...
            "Diaphragmvalve", "Endcaps", "Endcaps2", "Relief(angle)"
        };
        ...
        {
            "none",
            "Flangedaccesspoint", "Indicator", "CRT", "Diamond"
        };
        ...
        {
            XDocument xmlDocument = XMLGenerator();
        ...
        {
            List<PlantModel> plantEntity = new List<PlantModel>();
        ...
        public void PlantTypeClassification(PlantModel plantModel)
        {
            foreach (Shape shape in visioPage.Shapes)
            {
                ...
                if (EquipmentSymbolTypes.Contains(shapeReplace))
                {
                    ...
                    EquipmentAttribution(equipment, shape);
                ...
                else if (PipeSymbolTypes.Contains(shapeReplace))
                {
                    ...
                    PipingComponentAttribution(pipingComponent, shape);
                ...
                else if (InstrumentSymbolTypes.Contains(shapeReplace))
                {
                    ...
                    InstrumentAttribution(instrument, shape);
                ...
                else if (PipeLineTypes.Contains(shapeReplace))
                {
                    ...
                    PipeLineAttribution(pipeLine, shape);
                ...
                else if (InstrumentLineTypes.Contains(shapeReplace))
                {
                    ...
                    SignalAttribution(signalLine, shape);
        ...
        private void EquipmentAttribution(Equipment equipment, Shape shape)
        {
        ...
        private void PipingComponentAttribution(PipingComponent pipingComponent, Shape shape)
        {
            Extent extent = new Extent();
            Center center = new Center();
        ...
        private void InstrumentAttribution(Instrument instrument, Shape shape)
        {
            Extent extent = new Extent();
            Center center = new Center();
        ...
        private void PipeLineAttribution(PipeLine pipeLine, Shape shape)
        {
            Extent extent = new Extent();
            Center center = new Center();

            short iRow1 = (short)VisRowIndices.visRowXFormOut;
            short iRow2 = (short)VisRowIndices.visRowXForm1D;
            ...
            var strBeginX = shape.get_CellsSRC(
                    (short)VisSectionIndices.visSectionObject,
                    iRow2,
                    (short)VisCellIndices.vis1DBeginX
                    ).get_ResultStr(VisUnitCodes.visNoCast);
            int beginX = RemoveUnits(strBeginX);
            pipeLine.LineEndPoints.BeginPoints.BeginX = beginX;

            var strBeginY = shape.get_CellsSRC(
                    (short)VisSectionIndices.visSectionObject,
                    iRow2,
                    (short)VisCellIndices.vis1DBeginY
                    ).get_ResultStr(VisUnitCodes.visNoCast);
            int beginY = RemoveUnits(strBeginY);
            pipeLine.LineEndPoints.BeginPoints.BeginY = beginY;

            var strEndX = shape.get_CellsSRC(
                    (short)VisSectionIndices.visSectionObject,
                    iRow2,
                    (short)VisCellIndices.vis1DEndX
                    ).get_ResultStr(VisUnitCodes.visNoCast);
            int endX = RemoveUnits(strEndX);
            pipeLine.LineEndPoints.EndPoints.EndX = endX;

            var strEndY = shape.get_CellsSRC(
                    (short)VisSectionIndices.visSectionObject,
                    iRow2,
                    (short)VisCellIndices.vis1DEndY
                    ).get_ResultStr(VisUnitCodes.visNoCast);
            int endY = RemoveUnits(strEndY);
            pipeLine.LineEndPoints.EndPoints.EndY = endY;
        }

        private void SignalAttribution(SignalLine signalLine, Shape shape)
        {
            Extent extent = new Extent();
            Center center = new Center();
            ...
            int beginX = RemoveUnits(strBeginX);
            signalLine.LineEndPoints.BeginPoints.BeginX = beginX;
            ...
            int beginY = RemoveUnits(strBeginY);
            signalLine.LineEndPoints.BeginPoints.BeginY = beginY;
            ...
            int endX = RemoveUnits(strEndX);
            signalLine.LineEndPoints.EndPoints.EndX = endX;
            ...
            int endY = RemoveUnits(strEndY);
            signalLine.LineEndPoints.EndPoints.EndY = endY;
        }
